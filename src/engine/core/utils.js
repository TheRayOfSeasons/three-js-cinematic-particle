...
export function fillWithPoints(geometry, count) {
  ...
  let dir = new Vector3(1, 1, 1).normalize();
  let counter = 0;
  while(counter < count){
    let v = new Vector3(
      MathUtils.randFloat(bbox.min.x, bbox.max.x),
      MathUtils.randFloat(bbox.min.y, bbox.max.y),
      MathUtils.randFloat(bbox.min.z, bbox.max.z)
    );
    if (isInside(v)){
      points.push(v);
      counter++;
...
export function getCenter(sceneObjects) {
  let totalX = 0.0;
  let totalY = 0.0;
  let totalZ = 0.0;
  for(const sceneObject of sceneObjects) {
    totalX += sceneObject.position.x;
    totalY += sceneObject.position.y;
    totalZ += sceneObject.position.z;
  }
  return new Vector3(
    totalX / sceneObjects.length,
    totalY / sceneObjects.length,
    totalZ / sceneObjects.length,
  );
}

export function setAsymmetricInterval(callback, durations) {
